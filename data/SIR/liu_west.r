...
liu_west = function(y, sckm, n.particles, delta, prior)
{
  ...
  for (i in 1:n)
  {
    ...
    for (j in 1:n.particles) 
    {
      ...
      exp.nr = ceiling(hazard(sckm)$h) # Expected number of reactions/transitions
      w[j] = lweights[j,i] + sum(dbinom(y[i,], exp.nr, expit(m[p.i,j]), log=T))
    }

    # Resample particles
    w = renormalize.weights(w, log=T)
    kk = resample(w, method=resampling.function)

    # Propagate
    for (j in 1:n.particles)
    {
      sckm$theta = theta[,j,i+1] = m[,kk[j]] + h*ch%*%rnorm(2*sckm$r)
      sckm$X = X[,kk[j],i]
      tmp = tau_leap(sckm)
      X[,j,i+1] = tmp$X[2,]
      lweights[j,i+1] = sum(dbinom(y[i,], tmp$nr, expit(theta[p.i,j,i+1]), log=T)-
                            dbinom(y[i,], exp.nr, expit(    m[p.i,j,i  ]), log=T))
