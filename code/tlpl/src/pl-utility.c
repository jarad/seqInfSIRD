...
void suff_stat_update(const int *nRxns, const int *anRxns, const int *anY, const int *anHazardPart, 
                      double *adHyper) 
{
    int i,j;
    ...
    for (i=0; i<*nRxns; i++) 
    {
        ...
        adHyper[i+o1] += anRxns[i]-anY[i]; // Beta (beta)         - unobserved
        adHyper[i+o2] += anRxns[i];        // Gamma shape (alpha) - transitions
