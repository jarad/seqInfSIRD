...
/* Renormalizes the vector adWeights to sum to 1. If *log \ne 0, the adWeights are assumed to be log adWeights */
void renormalize(const int *n, const int *log, double *adWeights) 
{
  int i;
...
/* Performs multinomial resampling on the adWeights adWeights. Returns the anIndices for nSamples samples */
